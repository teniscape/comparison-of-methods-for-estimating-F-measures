  ...
  function(a, b, train_data, test_data, num, nsample, group, sd) {
    ...
    for (i in 1: num){
      ...
      for (k in 1: group){
        ...
        y_test_j <- test_data_j$Quality                
        X_test_l <- test_data_l[,-ncol(test_data_l)]
        y_test_l <- test_data_l$Quality                 
...
main_white_wine <-
  function(split, sd){
    ...
    White_wine <- read.table("data/white-wine/white-wine.txt")
    colnames(White_wine) <- c("F_acidity","V_acidity", "Acid", "Sugar", "Chlorides", "FS_dioxide", "TS_dioxide", "Density", "PH", "Sulphates", "Alcohol", "Quality")
    
    White_wine$Quality[(White_wine$Quality != 3)&(White_wine$Quality != 4)&(White_wine$Quality != 8)&(White_wine$Quality != 9)] <- 0
    White_wine$Quality[(White_wine$Quality == 3)|(White_wine$Quality == 4)|(White_wine$Quality == 8)|(White_wine$Quality == 9)] <- 1
    White_wine$Quality <- as.factor(as.character(White_wine$Quality))
    
    set.seed(sd)
    
    # Shuffle the original dataset by rows
    rows <- sample(nrow(White_wine))
    White_wine <- White_wine[rows, ] 
    
    train_data <- White_wine[1:split, ]
    test_data <- White_wine[(split+1):nrow(White_wine), ]
...
simu_time <- 1200
test_size <- 1000 
split_white_wine <- 3429

white_wine_main = main_white_wine(split_white_wine, seed)
white_wine_results = f_value_var(a, b, white_wine_main$train_data, white_wine_main$test_data, simu_time, test_size, group, seed)
