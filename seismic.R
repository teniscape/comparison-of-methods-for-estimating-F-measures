  ...
  function(a, b, train_data, test_data, num, nsample, group, sd) {
    ...
    for (i in 1: num){
      ...
      for (k in 1: group){
        ...
        y_test_j <- test_data_j$Class                 
        X_test_l <- test_data_l[,-ncol(test_data_l)]
        y_test_l <- test_data_l$Class                 
...
main_seismic <-
  function(split, sd){
    ...
    Seismic <- read.table("data/seismic/seismic.txt")
    colnames(Seismic) <- c("Seismic","Seismoacoustic", "Shift", "Genergy", "Gpuls", "Gdenergy", "Gdpuls", "Ghazard", "Nbumps", "Nbumps2", "Nbumps3", "Nbumps4", "Nbumps5", "Nbumps6", "Nbumps7", "Nbumps89", "Energy", "Maxenergy", "Class")
    ...
    Seismic$Class <- as.factor(as.character(Seismic$Class))
    
    set.seed(sd)
    
    # Shuffle the original dataset by rows
    rows <- sample(nrow(Seismic))
    Seismic <- Seismic[rows, ] 
    
    train_data <- Seismic[1:split, ]
    test_data <- Seismic[(split+1):nrow(Seismic), ]
...
seismic_main = main_seismic(split_seismic, seed)
seismic_results = f_value_var(a, b, seismic_main$train_data, seismic_main$test_data, simu_time, test_size, group, seed)
